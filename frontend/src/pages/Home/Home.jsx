...
const VIP_NUMBERS = [
  { num: '9898 1111 11', op: 'AIRTEL', price: '₹4,999' },
  { num: '9810 9999 99', op: 'JIO', price: '₹2,999' },
  { num: '9920 7777 77', op: 'VI', price: '₹999' },
  { num: '9876 0000 00', op: 'BSNL', price: '₹499' },
];
...
const CATEGORIES = [
  { title: 'PLATINUM', range: '₹4,000 – ₹5,000', icon: FaGem, color: 'text-[#e5e4e2]' },
  { title: 'GOLD', range: '₹2,000 – ₹3,999', icon: FaCrown, color: 'text-[#d4af37]' },
  { title: 'SILVER', range: '₹1,000 – ₹1,999', icon: FaStar, color: 'text-[#aaa9ad]' },
  { title: 'SPECIAL', range: '₹200 – ₹999', icon: FaHeart, color: 'text-[#d4af37]' },
];
...
const Home = () => {
  ...
  <div className="min-h-screen bg-[#050505] text-[#f5f0e1]">
    ...
    {/* ── HERO ── */}
    <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden px-6 pt-24 pb-16">
      {/* Background glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 right-1/4 w-[600px] h-[600px] rounded-full bg-[#d4af37]/5 blur-3xl" />
        <div className="absolute bottom-0 left-1/4 w-[400px] h-[400px] rounded-full bg-[#d4af37]/3 blur-3xl" />
      </div>
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center gap-12 z-10">
        ...
        <div ref={textRef} className="flex-1 text-center lg:text-left">
          <p className="text-[#d4af37] text-xs md:text-sm font-semibold tracking-[0.3em] uppercase mb-5">
            Exclusive Numbers <span className="mx-2 opacity-50">•</span> Exclusive You
          </p>
          ...
          <p className="text-base text-white/60 mb-10 max-w-md mx-auto lg:mx-0 leading-relaxed">
            Choose from India's most exclusive and memorable mobile numbers. Stand out with a number that defines you.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
            <Link
              to={ROUTES.NUMBERS}
              className="flex items-center justify-center gap-2 px-8 py-3.5 rounded text-sm font-bold uppercase tracking-wider text-black transition hover:opacity-90 active:scale-95"
        ...
        <div className="flex-1 flex justify-center">
          <div
            ref={logoRef}
            className="relative w-72 h-72 md:w-[420px] md:h-[420px] rounded-full flex items-center justify-center p-2"
            style={{ border: '1px solid rgba(212,175,55,0.25)', boxShadow: '0 0 120px rgba(212,175,55,0.15)' }}
          >
            {/* Outer rings */}
            ...
            <img 
              ...
              className="w-full h-full object-contain rounded-full relative z-10 drop-shadow-[0_0_30px_rgba(212,175,55,0.4)]"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-2 md:grid-cols-4 gap-8">
        {FEATURES.map(({ icon: Icon, title, desc }, i) => (
          <div key={i} className="flex flex-col items-center text-center gap-3">
            ...
            <h4 className="text-[#d4af37] text-xs font-bold tracking-[0.2em]">{title}</h4>
    ...
    <section className="py-24 px-6">
      <div className="max-w-7xl mx-auto text-center">
        <h2 className="text-3xl md:text-4xl font-display text-[#d4af37] mb-1">VIP SIM NUMBERS</h2>
        <GoldDivider />
        <p className="text-sm text-white/40 mb-14">Select your dream number from our premium collection</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {VIP_NUMBERS.map(({ num, op, price }, i) => (
            <div
              key={i}
              className="relative rounded-xl p-6 text-center group overflow-hidden transition duration-300 hover:scale-[1.02]"
              style={{
            ...
            >
              ...
              <p className="text-[11px] text-white/50 uppercase tracking-[0.3em] mb-3">{op}</p>
              <p className="text-xl font-semibold text-white mb-6">{price}</p>
        ...
        <Link
          to={ROUTES.NUMBERS}
          className="inline-flex items-center gap-2 px-8 py-3 border border-[#d4af37]/60 text-[#d4af37] text-xs font-bold uppercase tracking-widest rounded hover:bg-[#d4af37]/10 transition"
        >
    ...
    {/* ── POPULAR CATEGORIES ── */}
    <section id="categories" className="py-24 px-6 bg-[#060606] border-y border-[#d4af37]/10">
      <div className="max-w-7xl mx-auto text-center">
        <h2 className="text-3xl md:text-4xl font-display text-[#d4af37] mb-1">POPULAR CATEGORIES</h2>
        <GoldDivider />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-14">
          {CATEGORIES.map(({ title, range, icon: Icon, color }, i) => (
            <div
              key={i}
              className="rounded-xl p-8 text-center group cursor-pointer transition duration-300 hover:scale-[1.02]"
              style={{
                background: 'linear-gradient(180deg, rgba(22,22,22,0.9), rgba(10,10,10,0.95))',
                border: '1px solid rgba(212,175,55,0.18)',
              }}
            >
              <h3 className="text-[#d4af37] font-bold tracking-[0.25em] text-sm uppercase mb-1">{title}</h3>
              <p className="text-[10px] text-white/30 uppercase tracking-wider mb-4">Numbers</p>
              <Icon className={`${color} text-4xl mx-auto mb-6 group-hover:scale-110 transition-transform duration-300`} />
              <p className="text-xs text-white/60 mb-8">{range}</p>
              <button className="w-full py-2.5 text-xs font-bold uppercase tracking-widest border border-[#d4af37]/40 text-[#d4af37] rounded hover:bg-[#d4af37] hover:text-black transition">
                Explore
              </button>
            </div>
          ))}
    ...
    <div className="fixed bottom-6 right-6 z-50">
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="flex items-center gap-2 px-5 py-3 rounded-full text-white text-sm font-bold shadow-xl transition hover:scale-105"
